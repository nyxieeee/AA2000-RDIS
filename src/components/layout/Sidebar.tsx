...
const navItems = [
  ...
  { name: 'BIR Filing',   path: '/filing',    icon: FileText,        roles: ['Accountant'] },
        ...
        {navItems.filter(item => hasAccess(item.roles, user.role)).map((item) => {
          ...
          return (
            <Link
              ...
              className={cn(
                'flex items-center gap-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-300 group relative active:scale-[0.97]',
            ...
            >
              <item.icon className={cn(
                'h-5 w-5 shrink-0 transition-all duration-300 transform',
                isActive
              ...
              {!isCollapsed && (
                <>
                  ...
                  {item.name === 'Chat with AI' && (
                    <span className="ml-auto text-[10px] font-bold uppercase tracking-widest bg-blue-100 dark:bg-[#0f2040] text-blue-700 dark:text-blue-400 py-0.5 px-1.5 rounded shrink-0 animate-pulse">
                      Beta
                    </span>
                  )}
                </>
